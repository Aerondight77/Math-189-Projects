...
predictions <- predict(logRegMulti, newdata = valid_set, type = "response")
...
test_error <- mean(binary_predictions != valid_set$Private)
test_error
```
NEED TO JUSTIFY
ACCORDING TO CHATGPT: "a threshold of 0.5 is commonly used to classify observations into the two classes, "Yes" and "No". If the predicted probability of being in the "Private" class is greater than 0.5, the observation is classified as "Private," otherwise as "Public.""

##### (d) Fit an LDA to the same model, and report the test error.
```{r}
# DUMMY CODE, NEED ADJUSTMENT
library(MASS)

lda_model <- lda(Private ~ ., data = train_set)

lda_predictions <- predict(lda_model, newdata = valid_set)

test_error_lda <- mean(lda_predictions$class != valid_set$Private)
test_error_lda
```
...
```{r}
# DUMMY CODE, NEED ADJUSTMENT
qda_model <- qda(Private ~ ., data = train_set)

qda_predictions <- predict(qda_model, newdata = valid_set)

test_error_qda <- mean(qda_predictions$class != valid_set$Private)
test_error_qda
```
